...
const EditElement = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:4000/element/'+id)
    .then(response => response.json()
    .then(elInfo => {
      setTitle(elInfo.title);
      setSummary(elInfo.summary);
      setContent(elInfo.content);
      setCoverUrl(elInfo.cover);
    }))
  }, [])

  const updateElement = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost:4000/edit-element', {
        method: 'PUT',
  ...
  return (
    <div className='mt-6 border bg-white/10 backdrop-blur-md'>
      ...
      <form className='flex flex-col gap-4 py-6 px-8' onSubmit={updateElement}>
        <input
          required
          type='text'
          name='title'
